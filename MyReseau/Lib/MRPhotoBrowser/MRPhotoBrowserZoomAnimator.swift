//
//  MRPhotoBrowserZoomAnimator.swift
//  MRPhotoBrowser
//
//  Created by biz on 2020/4/26.
...
/// Zoom动画
open class MRPhotoBrowserZoomAnimator: NSObject, MRPhotoBrowserAnimatedTransitioning {
    ...
    private func snapshotsAndFrames(browser: MRPhotoBrowser) -> (UIView, UIView, CGRect, CGRect)? {
        ...
        guard let snap1 = fastSnapshot(with: previousView) else {
            MRPhotoBrowserLog.high("取不到前截图！")
            return nil
        }
        guard let snap2 = snapshot(with: cell.showContentView) else {
            MRPhotoBrowserLog.high("取不到后截图！")
            return nil
        }
