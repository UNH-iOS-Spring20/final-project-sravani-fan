//
//  MRPhotoBrowser.swift
//  MRPhotoBrowser
//
//  Created by biz on 2020/4/11.
//  Copyright © 2020 biz. All rights reserved.
//

import UIKit

open class MRPhotoBrowser: UIViewController, UIViewControllerTransitioningDelegate, UINavigationControllerDelegate {
    
    /// 通过本回调，把图片浏览器嵌套在导航控制器里
    public typealias PresentEmbedClosure = (MRPhotoBrowser) -> UINavigationController
    ...
    /// 自实现转场动画
    open lazy var transitionAnimator: MRPhotoBrowserAnimatedTransitioning = MRPhotoBrowserFadeAnimator()
    
    /// 滑动方向
    open var scrollDirection: MRPhotoBrowser.ScrollDirection {
    ...
    open var cellClassAtIndex: (_ index: Int) -> MRPhotoBrowserCell.Type {
    ...
    public typealias ReloadCellContext = (cell: MRPhotoBrowserCell, index: Int, currentIndex: Int)
    ...
    /// Cell将显示
    open var cellWillAppear: (MRPhotoBrowserCell, Int) -> Void {
        set { browserView.cellWillAppear = newValue }
        get { return browserView.cellWillAppear }
    }
    
    /// Cell将不显示
    open var cellWillDisappear: (MRPhotoBrowserCell, Int) -> Void {
        set { browserView.cellWillDisappear = newValue }
        get { return browserView.cellWillDisappear }
    }
    
    /// Cell已显示
    open var cellDidAppear: (MRPhotoBrowserCell, Int) -> Void {
        set { browserView.cellDidAppear = newValue }
        get { return browserView.cellDidAppear }
    }
    
    /// 主视图
    open lazy var browserView = MRPhotoBrowserView()
    
    /// 页码指示
    open var pageIndicator: MRPhotoBrowserPageIndicator?
    ...
    open func show(method: ShowMethod = .present(fromVC: nil, embed: nil)) {
        switch method {
        case .push(let inNC):
            let nav = inNC ?? MRPhotoBrowser.topMost?.navigationController
        ...
        case .present(let fromVC, let embed):
            ...
            let from = fromVC ?? MRPhotoBrowser.topMost
