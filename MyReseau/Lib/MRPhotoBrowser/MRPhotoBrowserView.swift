//
//  MRPhotoBrowserView.swift
//  MRPhotoBrowser
//
//  Created by biz on 2020/4/14.
//  Copyright © 2020 biz. All rights reserved.
//

import UIKit

open class MRPhotoBrowserView: UIView, UIScrollViewDelegate {
    
    /// 弱引用PhotoBrowser
    open weak var photoBrowser: MRPhotoBrowser?
    
    /// 询问当前数据总量
    open lazy var numberOfItems: () -> Int = { 0 }
    
    /// 返回可复用的Cell类。用户可根据index返回不同的类。本闭包将在每次复用Cell时实时调用。
    open lazy var cellClassAtIndex: (_ index: Int) -> MRPhotoBrowserCell.Type = { _ in
        MRPhotoBrowserImageCell.self
    }
    
    /// 刷新Cell数据。本闭包将在Cell完成位置布局后调用。
    open lazy var reloadCellAtIndex: (MRPhotoBrowser.ReloadCellContext) -> Void = { _ in }
    
    /// 自然滑动引起的页码改变时回调
    open lazy var didChangedPageIndex: (_ index: Int) -> Void = { _ in }
    
    /// Cell将显示
    open lazy var cellWillAppear: (MRPhotoBrowserCell, Int) -> Void = { _, _ in }
    
    /// Cell将不显示
    open lazy var cellWillDisappear: (MRPhotoBrowserCell, Int) -> Void = { _, _ in }
    
    /// Cell已显示
    open lazy var cellDidAppear: (MRPhotoBrowserCell, Int) -> Void = { _, _ in }
    
    /// 滑动方向
    open var scrollDirection: MRPhotoBrowser.ScrollDirection = .horizontal
    ...
    deinit {
        MRPhotoBrowserLog.high("deinit - \(self.classForCoder)")
    }
    ...
    /// 显示中的Cell
    open var visibleCells = [Int: MRPhotoBrowserCell]()
    
    /// 缓存中的Cell
    open var reusableCells = [String: [MRPhotoBrowserCell]]()
    
    /// 入队
    private func enqueue(cell: MRPhotoBrowserCell) {
    ...
    /// 出队，没缓存则新建
    private func dequeue(cellType: MRPhotoBrowserCell.Type, browser: MRPhotoBrowser) -> MRPhotoBrowserCell {
        var cell: MRPhotoBrowserCell
        let name = String(describing: cellType.classForCoder())
        if var array = reusableCells[name], array.count > 0 {
            MRPhotoBrowserLog.middle("命中缓存！\(name)")
            cell = array.removeFirst()
            reusableCells[name] = array
        } else {
            MRPhotoBrowserLog.middle("新建Cell! \(name)")
    ...
    open func resetCells() {
        ...
        for index in (pageIndex - 1)...(pageIndex + 1) {
            ...
            MRPhotoBrowserLog.middle("Required class name: \(String(describing: clazz))")
            MRPhotoBrowserLog.middle("index:\(index) 出列!")
